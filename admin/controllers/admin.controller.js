...
import { errorResponse, generateLoginToken, log1, removeFile, successResponse, uploadFile } from "../lib/general.js";
...
export const postCarOwnerDetails = async (req, res) => {
    try {
        ...
        let filter = {
            _id: new ObjectId(param?.carOwnerId),
        };
...
export const getCarMechanicPage = async (req, res) => {
    try {
        const admin = req.session.admin;
        ...
        log1(["Error in getCarMechanicPage----->", error]);
        return res.json(errorResponse(messages.unexpectedDataError));
    }
};

export const postAllCarMechanicList = async (req, res) => {
    try {
        const param = req?.body;
        const page = parseInt(req.body.currentPage) || Constants.DEFAULT_PAGE;
        const limit = parseInt(req.body.itemPerPage) || Constants.DEFAULT_LIMIT;
        ...
        log1(["Error in postAllCarMechanicList----->", error]);
        return res.json(errorResponse(messages.unexpectedDataError));
    }
};

export const postCarMechanicDetails = async (req, res) => {
    try {
        const param = req?.body;
        ...
        log1(["Error in postCarMechanicDetails----->", error]);
        return res.json(errorResponse(messages.unexpectedDataError));
    }
};

export const postCarMechanicDelete = async (req, res) => {
    try {
        const param = req?.body;
        ...
        log1(["Error in postCarMechanicDelete----->", error]);
        return res.json(errorResponse(messages.unexpectedDataError));
    }
};
...
export const postServiceDetails = async (req, res) => {
    try {
        ...
        let response = await Service.aggregate(aggregatePipeline);

        return res.status(200).json(successResponse("Service details get successfully!", response[0]));
    } catch (error) {
...
export const postServiceUpdate = async (req, res) => {
    try {
        ...
        const { serviceId, fullName, description, status } = req.body;
        ...
        if (status) {
            const validateStatus = Object.values(Constants.SERVICE_STATUS).includes(status);
            if (!validateStatus) {
        ...
        if (Object.keys(payload).length > 0) {
            const updateService = await Service.findOneAndUpdate(filter, payload);
            if (!updateService) {
                return res.status(400).json(errorResponse("Failed to update Service."));
            };
        };
